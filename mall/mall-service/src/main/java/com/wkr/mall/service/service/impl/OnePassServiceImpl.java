...
import com.wkr.admin.common.constants.OnePassConstants;
...
import com.wkr.admin.common.utils.ValidateFormUtil;
import com.wkr.admin.common.model.express.Express;
import com.wkr.admin.common.vo.OnePassLoginVo;
import com.wkr.admin.common.vo.OnePassLogisticsQueryVo;
import com.wkr.mall.service.service.ExpressService;
import com.wkr.mall.service.service.OnePassService;
import com.wkr.mall.service.service.SystemConfigService;
import com.wkr.mall.service.util.OnePassUtil;
...
public class OnePassServiceImpl implements OnePassService {
    ...
    public JSONObject mealList(String type) {
        if (!validateMealType(type)) throw new MallException("请选择正确的套餐类型");
    ...
    public JSONObject mealCode(MealCodeRequest request) {
        if (!validateMealType(request.getType())) throw new MallException("请选择正确的套餐类型");
        // 获取用户详情
        String token = onePassUtil.getToken();
        HashMap<String, String> header = onePassUtil.getCommonHeader(token);
        JSONObject jsonObject = onePassUtil.postFrom(OnePassConstants.ONE_PASS_API_URL + OnePassConstants.USER_INFO_URI, null, header);
        ...
        if (!isGrant) throw new MallException("请先开通对应的服务");
    ...
    public Boolean serviceOpen(ServiceOpenRequest request) {
        if (!validateMealType(request.getType())) throw new MallException("请选择正确的服务类型");
        Boolean open = false;
        switch (request.getType()) {
            case OnePassConstants.ONE_PASS_MEAL_TYPE_SMS:// 短信开通
                if (StrUtil.isBlank(request.getSign())) throw new MallException("签名不能为空");
    ...
    public JSONObject userRecord(OnePassUserRecordRequest request) {
        if (!validateMealType(request.getType())) throw new MallException("请选择正确的记录类型");
        MultiValueMap<String, Object> params = new LinkedMultiValueMap<>();
        params.add("type", request.getType());
...
//            int status = 3;// 查询全部状态
    ...
    private Boolean expressOpen(ServiceOpenRequest request) {
        ...
        if (ObjectUtil.isNull(express)) throw new MallException("没有找到对应的快递公司");
        ...
        if (Boolean.FALSE.equals(execute)) throw new MallException("保存平台电子面单打印信息失败");
