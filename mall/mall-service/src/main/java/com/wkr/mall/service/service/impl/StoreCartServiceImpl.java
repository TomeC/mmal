...
import com.wkr.admin.common.request.CartNumRequest;
import com.wkr.admin.common.request.CartRequest;
import com.wkr.admin.common.request.CartResetRequest;
...
import com.wkr.admin.common.response.CartInfoResponse;
...
import com.wkr.mall.service.dao.StoreCartDao;
    ...
    */
    ...
    public String saveCate(CartRequest storeCartRequest) {
        ...
        if (ObjectUtil.isNull(product) || product.getIsDel() || !product.getIsShow()) {
            throw new MallException("未找到对应商品");
        }
        ...
        if (CollUtil.isEmpty(attrValues)) {
            throw new MallException("未找到对应的商品SKU");
        }
    ...
    public Boolean resetCart(CartResetRequest resetRequest) {
        ...
        if (ObjectUtil.isNull(storeCart)) throw new MallException("购物车不存在");
    ...
    public Boolean updateCartNum(Integer id, Integer number) {
        if (ObjectUtil.isNull(number)) throw new MallException("商品数量不合法");
        if (number <=0 || number > 99) throw new MallException("商品数量不能小于1大于99");
        StoreCart storeCart = getById(id);
        if (ObjectUtil.isNull(storeCart)) throw new MallException("当前购物车不存在");
