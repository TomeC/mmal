...
import com.wkr.admin.common.constants.UserConstants;
...
import com.wkr.admin.common.response.StoreBargainUserHelpResponse;
import com.wkr.admin.common.model.user.User;
import com.wkr.admin.common.model.user.UserToken;
import com.wkr.mall.service.dao.StoreBargainUserHelpDao;
...
public class StoreBargainUserHelpServiceImpl extends ServiceImpl<StoreBargainUserHelpDao, StoreBargainUserHelp> implements StoreBargainUserHelpService {
    ...
    public Map<String, Object> help(BargainFrontRequest request) {
        if (ObjectUtil.isNull(request.getBargainUserId())) {
            throw new MallException("砍价活动id不能为空");
        }
        ...
        if (currentTimeMillis < storeBargain.getStartTime()) {
            throw new MallException("砍价活动未开始");
        }
        ...
        if (ObjectUtil.isNull(storeBargainUser)) {
            throw new MallException("砍价商品用户信息不存在");
        }
