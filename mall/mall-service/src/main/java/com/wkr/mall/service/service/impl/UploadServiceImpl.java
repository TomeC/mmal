...
import com.wkr.admin.common.vo.FileResultVo;
import com.wkr.admin.common.vo.UploadCommonVo;
...
import com.wkr.admin.common.utils.UploadUtil;
import com.wkr.admin.common.model.system.SystemAttachment;
...
//        String rootPath = (systemConfigService.getValueByKey(Constants.UPLOAD_ROOT_PATH_CONFIG_KEY) + "/").replace(" ", "").replace("//", "/");
        String rootPath = mallConfig.getImagePath().replace(" ", "").replace("//", "/");
        ...
        if( fileSize > uploadCommonVo.getSize()){
            throw new MallException("最大允许上传" + uploadCommonVo.getSize() + " MB的文件, 当前文件大小为 " + fs + " MB");
        }

        // 判断文件的后缀名是否符合规则
        if (StringUtils.isNotEmpty(uploadCommonVo.getExtStr())) {
            // 切割文件扩展名
            List<String> extensionList = MallUtil.stringToArrayStr(uploadCommonVo.getExtStr());
            if (extensionList.size() > 0) {
                //判断
                if (!extensionList.contains(extName)) {
                    throw new MallException("上传文件的类型只能是：" + uploadCommonVo.getExtStr());
                }
            } else {
                throw new MallException("上传文件的类型只能是：" + uploadCommonVo.getExtStr());
            }
...
//                multipartFile.transferTo(file);
            ...
            case 2:
                ...
                try{
                    ...
                    String webPathQn = mallConfig.getImagePath();
...
//                            systemAttachment.getSattDir(), webPathQn + "/" + systemAttachment.getSattDir(), systemAttachment.getAttId());   //异步处理
            ...
            case 3:
                ...
                cloudVo.setRegion(systemConfigService.getValueByKeyException(pre+"StorageRegion"));
                try{
                    String webPathAl = mallConfig.getImagePath();
            ...
            case 4:
                ...
                try{
                    String webPathTx = mallConfig.getImagePath();
