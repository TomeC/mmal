...
import com.wkr.admin.common.vo.MyRecord;
import com.wkr.admin.common.constants.BargainConstants;
import com.wkr.admin.common.constants.Constants;
import com.wkr.admin.common.constants.ProductConstants;
...
import com.wkr.admin.common.model.product.StoreProductAttr;
import com.wkr.admin.common.model.product.StoreProductAttrValue;
import com.wkr.admin.common.model.product.StoreProductDescription;
import com.wkr.admin.common.model.user.User;
import com.wkr.mall.service.dao.StoreBargainDao;
    ...
    */
    ...
    public boolean saveBargain(StoreBargainRequest request) {
        ...
        if (tempPrice.compareTo(multiply) < 0) {
            ...
            throw new MallException("必须保证每个人最少砍1分钱");
        }
    ...
    public boolean updateBargain(StoreBargainRequest request) {
        if (ObjectUtil.isNull(request.getId())) {
            throw new MallException("砍价商品id不能为空");
        }
    ...
    public StoreProductInfoResponse getAdminDetail(Integer bargainId) {
        ...
        if (ObjectUtil.isNull(storeBargain) || storeBargain.getIsDel()) {
            throw new MallException("未找到对应砍价商品信息");
        }
    ...
    public BargainDetailH5Response getH5Detail(Integer id) {
        ...
        if (!storeBargain.getStatus()) {
            throw new MallException("砍价商品已下架");
        }
        ...
        if (CollUtil.isEmpty(storeProductAttrValuesBargain)) {
            throw new MallException("砍价商品规格属性值未找到");
        }
    ...
    public MyRecord start(BargainFrontRequest request) {
        ...
        if (ObjectUtil.isNull(storeBargain) || storeBargain.getIsDel()) {
            throw new MallException("对应的砍价商品不存在");
        }
        if (!storeBargain.getStatus()) {
            throw new MallException("砍价商品已下架");
        }
        if (storeBargain.getQuota() <= 0 || storeBargain.getStock() <= 0) {
            throw new MallException("砍价商品已售罄");
        }
        ...
        if (CollUtil.isNotEmpty(historyList)) {
            ...
            if (CollUtil.isNotEmpty(collect)) {
                throw new MallException("请先完成当前砍价活动");
            }
            // 判断是否达到参与砍价活动上限
        ...
        if (!save) {
            throw new MallException("参与砍价失败");
        }
    ...
    public StoreBargain getByIdException(Integer id) {
        ...
        if (ObjectUtil.isNull(storeBargain)) {
            logger.error("砍价商品id {} 不存在或未开启", id);
            throw new MallException("砍价商品不存在或未开启");
        }
        return storeBargain;
    }
    ...
    public Boolean operationStock(Integer id, Integer num, String type) {
        ...
        if (!update) {
            throw new MallException("更新砍价商品库存失败,商品id = " + id);
        }
        return update;
    }
