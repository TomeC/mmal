...
import com.wkr.admin.common.constants.*;
import com.wkr.admin.common.request.*;
import com.wkr.admin.common.response.*;
import com.wkr.admin.common.vo.*;
import com.wkr.admin.common.exception.MallException;
import com.wkr.admin.common.model.bargain.StoreBargain;
import com.wkr.admin.common.model.bargain.StoreBargainUser;
import com.wkr.admin.common.model.cat.StoreCart;
import com.wkr.admin.common.model.combination.StoreCombination;
import com.wkr.admin.common.model.coupon.StoreCouponUser;
import com.wkr.admin.common.model.express.Express;
import com.wkr.admin.common.model.express.ShippingTemplates;
...
import com.wkr.admin.common.model.order.StoreOrderInfo;
import com.wkr.admin.common.model.order.StoreOrderStatus;
import com.wkr.admin.common.model.product.StoreProduct;
import com.wkr.admin.common.model.product.StoreProductAttrValue;
import com.wkr.admin.common.model.seckill.StoreSeckill;
import com.wkr.admin.common.model.seckill.StoreSeckillManger;
import com.wkr.admin.common.model.sms.SmsTemplate;
import com.wkr.admin.common.model.system.SystemAdmin;
import com.wkr.admin.common.model.system.SystemNotification;
import com.wkr.admin.common.model.system.SystemStore;
import com.wkr.admin.common.model.system.SystemUserLevel;
import com.wkr.admin.common.model.user.User;
import com.wkr.admin.common.model.user.UserAddress;
...
import com.wkr.mall.service.delete.OrderUtils;
import com.wkr.mall.service.service.*;
...
public class OrderServiceImpl implements OrderService {
    ...
    public Boolean reply(StoreProductReplyAddRequest request) {
        if (StrUtil.isBlank(request.getOrderNo())) {
            throw new MallException("订单号参数不能为空");
        }
    ...
    public Boolean refundApply(OrderRefundApplyRequest request) {
        ...
        if (null == existStoreOrder) throw new MallException("支付订单不存在");
        ...
        if (!execute) throw new MallException("申请退款失败");
        return execute;
    }
    ...
    public Object expressOrder(String orderId) {
        ...
        if (ObjectUtil.isNull(existOrder)) throw new MallException("未找到该订单信息");
    ...
    public MyRecord preOrder(PreOrderRequest request) {
        if (CollUtil.isEmpty(request.getOrderDetails())) {
            throw new MallException("预下单订单详情列表不能为空");
        }
        ...
        // 缓存订单
        String key = user.getUid() + DateUtil.getNowTime().toString()+ MallUtil.getUuid();
    ...
    public PreOrderResponse loadPreOrder(String preOrderNo) {
        ...
        if (!exists) {
            throw new MallException("预下单订单不存在");
        }
    ...
    public MyRecord createOrder(CreateOrderRequest request) {
        ...
        // 检测支付方式
        if (!orderUtils.checkPayType(request.getPayType())) throw new MallException("暂不支持该支付方式，请刷新页面或者联系管理员");

        if (request.getPayType().equals(PayConstants.PAY_TYPE_WE_CHAT)) {
            // 检测支付渠道
            if (StrUtil.isBlank(request.getPayChannel())) throw new MallException("支付渠道不能为空!");
            if (!OrderUtils.checkPayChannel(request.getPayChannel())) throw new MallException("支付渠道不存在!");
        }
        ...
        if (request.getShippingType() == 1) { // 快递配送
            ...
            if (ObjectUtil.isNull(systemStore) || systemStore.getIsDel() || !systemStore.getIsShow()) {
                throw new MallException("暂无门店无法选择门店自提");
            }
        ...
        if (ObjectUtil.isNotNull(orderInfoVo.getSeckillId()) && orderInfoVo.getSeckillId() > 0) {
            ...
            if (storeSeckill.getStatus().equals(0)) {
                throw new MallException("秒杀商品已关闭");
            }
    ...
    private List<MyRecord> validateProductStock(OrderInfoVo orderInfoVo, User user) {
        ...
        orderDetailList.forEach(e -> {
            ...
            if (ObjectUtil.isNull(storeProduct)) {
                throw new MallException("购买的商品信息不存在");
            }
            if (storeProduct.getIsDel()) {
                throw new MallException("购买的商品已删除");
            }
            if (!storeProduct.getIsShow()) {
                throw new MallException("购买的商品已下架");
            }
            ...
            if (ObjectUtil.isNull(attrValue)) {
                throw new MallException("购买的商品规格信息不存在");
            }
    ...
    private OrderInfoVo validatePreOrderRequest(PreOrderRequest request, User user) {
        ...
        if ("buyNow".equals(request.getPreOrderType())) {// 立即购买
            ...
            if (detailRequest.getSeckillId() > 0) {// 秒杀
                ...
                if (ObjectUtil.isNull(attrValue)) {
                    throw new MallException("商品规格信息不存在，请刷新后重新选择");
                }
                if (attrValue.getStock() < detailRequest.getProductNum()) {
                    throw new MallException("商品规格库存不足，请刷新后重新选择");
                }
    ...
    private MyRecord commonValidateSeckill(StoreSeckill storeSeckill, StoreProductAttrValue seckillAttrValue, User user, Integer productNum) {
        ...
        if (ObjectUtil.isNull(seckillManger)) {
            throw new MallException("秒杀时段不存在");
        }
        // 判断日期是否过期
        ...
        if (CollUtil.isNotEmpty(userCurrentDaySecKillOrders)) {
            ...
            if (unPayOrders.size() > 0) {
                throw new MallException("您有秒杀待支付订单，请支付后再购买");
            }
    ...
    private OrderInfoDetailVo validatePreOrderBargain(PreOrderDetailRequest detailRequest, User user) {
        if (detailRequest.getBargainUserId() <= 0) {
            throw new MallException("用户砍价活动id必须大于0");
        }
        // 砍价部分判断
    ...
    private MyRecord commonValidateBargain(StoreBargain storeBargain, StoreProductAttrValue bargainAttrValue, Integer bargainUserId, User user, Integer productNum) {
        ...
        if (!bargainUser.getStatus().equals(3)) {
            if (bargainUser.getBargainPriceMin().compareTo(bargainUser.getBargainPrice().subtract(bargainUser.getPrice())) != 0) {
                throw new MallException("请先完成砍价活动");
            }
        }
        // 判断是否有订单
        ...
        if (ObjectUtil.isNotNull(bargainOrder)) {
            if (!bargainOrder.getPaid()) {
                throw new MallException("订单已创建，尚未支付");
            }
            throw new MallException("该砍价活动已创建了订单");
        }
    ...
    private MyRecord commonValidateCombination(StoreCombination storeCombination, StoreProductAttrValue combinationAttrValue, User user, Integer productNum) {
        ...
        if (ObjectUtil.isNull(productAttrValue)) {
            throw new MallException("拼团主商品规格不存在");
        }
        if (productAttrValue.getStock() <= 0 || productNum > productAttrValue.getStock()) {
            throw new MallException("拼团主商品规格库存不足");
        }
    ...
    private List<OrderInfoDetailVo> validatePreOrderAgain(PreOrderDetailRequest detailRequest, User user) {
        ...
        if (StrUtil.isBlank(detailRequest.getOrderNo())) {
            throw new MallException("再次购买订单编号不能为空");
        }
        ...
        if ("true".equals(postageSwitchString) && ("0".equals(storeFreePostageString) || orderInfoVo.getProTotalFee().compareTo(new BigDecimal(storeFreePostageString)) >= 0)) {
            ...
            for (Map.Entry<Integer, MyRecord> m : proMap.entrySet()) {
                ...
                if (ObjectUtil.isNull(shippingTemplatesRegion)) {
                    throw new MallException("计算运费时，未找到全国运费配置");
                }
    ...
    private ComputedOrderPriceResponse computedPrice(OrderComputedPriceRequest request, OrderInfoVo orderInfoVo, User user) {
        ...
        if (ObjectUtil.isNull(request.getCouponId()) || request.getCouponId() <= 0) {
            ...
            throw new MallException("营销活动商品无法使用优惠券");
        } else if (orderInfoVo.getIsVideo()) {
            throw new MallException("视频号商品无法使用优惠券");
        } else {
            // 判断优惠券是否可以使用
            ...
            if (ObjectUtil.isNull(storeCouponUser) || !storeCouponUser.getUid().equals(user.getUid())) {
                throw new MallException("优惠券领取记录不存在！");
            }
