...
import com.wkr.admin.common.model.user.*;
...
import com.wkr.admin.common.model.coupon.StoreCoupon;
import com.wkr.admin.common.model.coupon.StoreCouponUser;
import com.wkr.admin.common.model.order.StoreOrder;
import com.wkr.admin.common.model.record.UserVisitRecord;
import com.wkr.admin.common.model.system.SystemUserLevel;
import com.wkr.admin.common.page.CommonPage;
import com.wkr.admin.common.token.FrontTokenComponent;
import com.wkr.admin.common.utils.CommonUtil;
...
import com.wkr.admin.common.vo.dateLimitUtilVo;
import com.wkr.mall.service.dao.UserDao;
...
public class UserServiceImpl extends ServiceImpl<UserDao, User> implements UserService {
    ...
    public PageInfo<UserResponse> getList(UserSearchRequest request, PageParamRequest pageParamRequest) {
        ...
        if (!StringUtils.isBlank(request.getLabelId())) {
            String tagIdSql = MallUtil.getFindInSetSql("u.tag_id", request.getLabelId());
        ...
        for (User user : userList) {
            ...
            userResponse.setPhone(MallUtil.maskMobile(userResponse.getPhone()));
    ...
    public Boolean updateIntegralMoney(UserOperateIntegralMoneyRequest request) {
        ...
        if (request.getMoneyType().equals(2) && request.getMoneyValue().compareTo(BigDecimal.ZERO) != 0) {
            if (user.getNowMoney().subtract(request.getMoneyValue()).compareTo(BigDecimal.ZERO) < 0) {
                throw new MallException("余额扣减后不能小于0");
            }
        }
    ...
    public Boolean group(String id, String groupIdValue) {
        if (StrUtil.isBlank(id)) throw new MallException("会员编号不能为空");
        if (StrUtil.isBlank(groupIdValue)) throw new MallException("分组id不能为空");

        //循环id处理
        List<Integer> idList = MallUtil.stringToArray(id);
        idList = idList.stream().distinct().collect(Collectors.toList());
        List<User> list = getListInUid(idList);
        if (CollUtil.isEmpty(list)) throw new MallException("没有找到用户信息");
    ...
    public Boolean updatePhoneVerify(UserBindingPhoneUpdateRequest request) {
        ...
        if (!user.getPhone().equals(request.getPhone())) {
            throw new MallException("手机号不是当前用户手机号");
        }
    ...
    public UserCenterResponse getUserCenter() {
        ...
        if (ObjectUtil.isNull(currentUser)) {
            throw new MallException("您的登录已过期，请先登录");
        }
    ...
    public Boolean tag(String id, String tagIdValue) {
        if (StrUtil.isBlank(id)) throw new MallException("会员编号不能为空");
        if (StrUtil.isBlank(tagIdValue)) throw new MallException("标签id不能为空");
...
//    }
    ...
    public void bindSpread(Integer spreadUid) {
        ...
        User user = getInfo();
        if (ObjectUtil.isNull(user)) {
            throw new MallException("当前用户未登录,请先登录");
        }
    ...
    public Boolean editSpread(UserUpdateSpreadRequest request) {
        ...
        if (spreadUser.getSpreadUid().equals(userId)) {
            throw new MallException("当前用户已是推广人的上级");
        }
    ...
    public Boolean updateUserPhone(Integer id, String phone) {
        ...
        if (!matchPhone) {
            throw new MallException("手机号格式错误，请输入正确得手机号");
        }
        User user = getById(id);
        if (ObjectUtil.isNull(user)) {
            throw new MallException("对应用户不存在");
        }
        if (phone.equals(user.getPhone())) {
            throw new MallException("手机号与之前一致");
        }
    ...
    public Boolean updateUserLevel(UpdateUserLevelRequest request) {
        ...
        if (user.getLevel().equals(request.getLevelId())) {
            throw new MallException("用户等级与修改前相同");
        }
        ...
        if (ObjectUtil.isNull(systemUserLevel)) {
            throw new MallException("系统会员等级不存在，请先配置");
        }
        ...
        if (ObjectUtil.isNull(userLevel)) {
            throw new MallException("用户会员等级不存在，请检查用户数据");
        }
        if (userLevel.getGrade() > systemUserLevel.getGrade()) {
            if (ObjectUtil.isNull(request.getIsSub())) {
                throw new MallException("降低用户等级时，请选择是否扣除用户经验值");
            }
        }
