...
import com.wkr.admin.common.request.SystemUserLevelRequest;
import com.wkr.admin.common.request.SystemUserLevelUpdateShowRequest;
import com.wkr.mall.service.dao.SystemUserLevelDao;
import com.wkr.mall.service.service.SystemAttachmentService;
import com.wkr.mall.service.service.SystemUserLevelService;
import com.wkr.mall.service.service.UserLevelService;
...
public class SystemUserLevelServiceImpl extends ServiceImpl<SystemUserLevelDao, SystemUserLevel> implements SystemUserLevelService {
    ...
    private void checkLevel(SystemUserLevelRequest request) {
        ...
        if (ObjectUtil.isNotNull(temp)) {
            throw new MallException("用户等级名称重复");
        }
        // 校验等级级别
        lqw.clear();
        ...
        temp = dao.selectOne(lqw);
        if (ObjectUtil.isNotNull(temp)) {
            throw new MallException("用户等级级别重复");
        }
        ...
        if (request.getGrade() > 1) {
            ...
            if (ObjectUtil.isNotNull(temp) && temp.getExperience() >= request.getExperience()) {
                throw new MallException("当前等级的经验不能比上一级别的经验低");
            }
        }
        lqw.clear();
        ...
        temp = dao.selectOne(lqw);
        if (ObjectUtil.isNotNull(temp) && temp.getExperience() <= request.getExperience()) {
            throw new MallException("当前等级的经验不能比下一级别的经验高");
        }
    }
    ...
    public Boolean delete(Integer id) {
        SystemUserLevel level = getById(id);
        if (ObjectUtil.isNull(level) || level.getIsDel()) {
            throw new MallException("系统等级不存在");
        }
        level.setIsDel(true);
        return transactionTemplate.execute(e -> {
